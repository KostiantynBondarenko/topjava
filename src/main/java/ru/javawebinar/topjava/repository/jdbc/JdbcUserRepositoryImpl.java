...
public class JdbcUserRepositoryImpl implements UserRepository {
    ...
    public User save(User user) {
        ...
        if (user.isNew()) {
            Number newId = insertUser.executeAndReturnKey(parameterSource);
            user.setId(newId.intValue());
        } else {
            if (namedParameterJdbcTemplate.update(
                    "UPDATE users " +
                            "SET name=:name, email=:email, password=:password, registered=:registered, enabled=:enabled, calories_per_day=:caloriesPerDay " +
